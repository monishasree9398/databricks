...
export const ClassDistributionChart: React.FC<ClassDistributionChartProps> = ({
  ...
  return (
    <Card className="p-6 md:p-7">
      ...
      {/* Sunken Neumorphic Segmented Progress Bar */}
      <div className="h-4 w-full rounded-full bg-[#EEF2F6] shadow-neu-pressed p-0.5 flex gap-1 overflow-hidden my-6">
        <div
          style={{ width: `${distribution.readyPercentage}%` }}
          className="h-full rounded-l-full bg-emerald-500 transition-all duration-500 relative cursor-pointer shadow-sm"
          title={`Ready: ${distribution.ready} students (${distribution.readyPercentage}%)`}
        />
        <div
          style={{ width: `${distribution.almostReadyPercentage}%` }}
          className="h-full bg-brand-orange transition-all duration-500 relative cursor-pointer shadow-sm"
        ...
        <div
          ...
          className="h-full rounded-r-full bg-amber-500 transition-all duration-500 relative cursor-pointer shadow-sm"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {/* Tier 1: Ready */}
        <div className="p-5 rounded-2xl bg-[#EEF2F6] shadow-neu-sm flex flex-col justify-between">
          ...
          <div className="mt-3.5">
            <span className="text-3xl font-black text-slate-900 font-mono">{distribution.ready}</span>
            <span className="text-xs text-slate-500 ml-1 font-mono">students</span>
          </div>
        ...
        {/* Tier 2: Almost Ready */}
        <div className="p-5 rounded-2xl bg-[#EEF2F6] shadow-neu-sm flex flex-col justify-between" style={{ outline: '1px solid rgba(255, 85, 0, 0.2)' }}>
          <div className="flex items-center justify-between">
          ...
          <div className="mt-3.5">
            <span className="text-3xl font-black text-slate-900 font-mono">{distribution.almostReady}</span>
            <span className="text-xs text-slate-500 ml-1 font-mono">students</span>
          </div>
          <p className="text-[11px] text-brand-orange mt-2 font-medium">
        ...
        <div className="p-5 rounded-2xl bg-[#EEF2F6] shadow-neu-sm flex flex-col justify-between">
          ...
          <div className="mt-3.5">
            <span className="text-3xl font-black text-slate-900 font-mono">{distribution.needsSupport}</span>
            <span className="text-xs text-slate-500 ml-1 font-mono">students</span>
          </div>
          <p className="text-[11px] text-slate-500 mt-2 font-medium">
