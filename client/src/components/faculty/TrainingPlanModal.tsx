...
import { Sparkles, Copy, Check, Send, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="AI-Generated Cohort Training Plan" maxWidth="2xl">
      <div className="space-y-5">
        {/* Banner Header */}
        <div className="p-4 rounded-xl bg-orange-50 border border-orange-200 flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-white text-brand-orange border border-orange-200 font-mono">
                CURRICULUM AI
              </span>
              <span className="text-[11px] font-bold text-brand-orange uppercase font-mono">CS-401 Cohort</span>
            </div>
            <h4 className="text-base font-bold text-slate-900 font-sans">{plan.title}</h4>
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
          {plan.modules?.map((m: any) => (
            <div key={m.week} className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-brand-orange font-mono uppercase">
                  Week 0{m.week}
                </span>
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                {m.topics?.map((topic: string) => (
                  <span key={topic} className="px-2 py-0.5 rounded text-[11px] font-mono bg-white text-slate-700 border border-slate-200">
                    {topic}
                  </span>
                ))}
              </div>
        ...
        <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
          <button
            onClick={handleCopy}
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200 transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={deployed}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold bg-brand-orange text-white hover:bg-brand-orangeHover transition-all shadow-sm disabled:opacity-75"
            >
              {deployed ? (
                <>
